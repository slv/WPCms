...
Class WPCmsImageProField Extends WPCmsField {
  ...
  public function renderInnerInput ($post, $data = array())
  {
    ...
    <div class="image-pro-wrapper"><?php if ($data['value'] != ''): $images = explode(',', $data['value']); foreach ($images as $image): ?>
    ...
    <input type="button" value="<?php _e('Select Image', WPCmsStatus::getStatus()->getData('textdomain')); ?>" class="btn btn-default btn-xs image-pro-button" />
    <input type="button" value="<?php _e('Remove Image', WPCmsStatus::getStatus()->getData('textdomain')); ?>" class="btn btn-danger btn-xs image-pro-delete"<?php if ($data['value'] == '') echo ' style="display:none;"'; ?> />
