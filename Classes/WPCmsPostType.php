...
Class WPCmsPostType {
  ...
  function register () {
    ...
    WPCmsStatus::getStatus()->setData('custom_post_type_' . $this->post_type, $this);
    ...
    foreach ($this->custom_fields as $custom_field) {
      ...
      foreach ($custom_field['fields'] as $field) {
        ...
        $field->addActionRegister($this->post_type);
      }
    }
  ...
  function addCustomFields ($fields) {
    foreach ($fields as $key => $value) {
      ...
      if ($key == 'common') {
        if (!isset($this->custom_fields['common']))
          $this->custom_fields['common'] = $value;
        else
          $this->custom_fields['common']['fields'] = array_merge($this->custom_fields['common']['fields'], $value);
      } else {
  ...
  function custom_fields_save ($idPost) {
    ...
    elseif (!empty($_POST['action']) && $_POST['action'] == 'inline-save') {
      return $idPost;
    }
